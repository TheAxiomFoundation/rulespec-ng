...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "data", "ng", "tests"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "LICENSE",
...
def test_only_nigeria_namespace_present() -> None:
    """Individual income tax is uniform federal law: the only jurisdiction directory is ng/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"ng"}, f"unexpected jurisdiction dirs: {names - {'ng'}}"


def test_ng_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "ng" / marker).is_dir(), f"missing ng/{marker}"
...
def test_oracle_index_is_ng_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "ng"


def test_source_map_is_ng_scoped() -> None:
    payload = json.loads(
